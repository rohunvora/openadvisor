import { pgTable, text, uuid, timestamp, integer, jsonb, boolean, decimal, index, pgEnum, PgEnum } from 'drizzle-orm/pg-core';
import { relations } from 'drizzle-orm';
import type { InferSelectModel, InferInsertModel } from 'drizzle-orm';

// Enums
export const userTypeEnum: PgEnum<['kol', 'project']> = pgEnum('user_type', ['kol', 'project']);
export const offerStatusEnum: PgEnum<['draft', 'active', 'expired', 'cancelled']> = pgEnum('offer_status', ['draft', 'active', 'expired', 'cancelled']);
export const dealStatusEnum: PgEnum<['pending', 'accepted', 'rejected', 'revoked']> = pgEnum('deal_status', ['pending', 'accepted', 'rejected', 'revoked']);
export const vestingStatusEnum: PgEnum<['pending', 'active', 'completed', 'revoked']> = pgEnum('vesting_status', ['pending', 'active', 'completed', 'revoked']);
...
export const offers = pgTable('offers', {
  id: uuid('id').defaultRandom().primaryKey(),
  projectId: uuid('project_id').references(() => users.id).notNull(),
...
export const deals = pgTable('deals', {
  ...
  kolId: uuid('kol_id').references(() => users.id).notNull(),
  projectId: uuid('project_id').references(() => users.id).notNull(),
  
  // Deal specifics
  agreedTokenAmount: decimal('agreed_token_amount', { precision: 20, scale: 0 }).notNull(),
  ...
  vestingAccountIdx: index('vesting_account_idx').on(table.vestingAccountAddress),
}));
...
export const vestingSchedules = pgTable('vesting_schedules', {
  ...
  releasedAmount: decimal('released_amount', { precision: 20, scale: 0 }).default('0').notNull(),
...
export const disclosureLogs = pgTable('disclosure_logs', {
  id: uuid('id').defaultRandom().primaryKey(),
  dealId: uuid('deal_id').references(() => deals.id).notNull(),
  kolId: uuid('kol_id').references(() => users.id).notNull(),
